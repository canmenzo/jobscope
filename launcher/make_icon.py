...
OUT_OPEN = Path(__file__).resolve().parent / "jobscope-open.ico"
SIZES = [(16, 16), (24, 24), (32, 32), (48, 48), (64, 64), (128, 128), (256, 256)]

BG_TOP, BG_BOTTOM = (28, 34, 48), (15, 17, 21)
BLUE = (59, 130, 246, 255)
GREEN = (52, 211, 153, 255)
# The "open the last result" shortcut gets the same reticle in the accent
# colour, so the two taskbar buttons are never mistaken for each other.
OPEN_RING = (52, 211, 153, 255)
OPEN_DOT = (59, 130, 246, 255)
...
def gradient(size, top, bottom):
    ...
    for y in range(size):
        t = y / (size - 1)
        px[0, y] = tuple(round(a + (b - a) * t) for a, b in zip(top, bottom, strict=True))
...
def render(ring, dot):
    ...
    d.ellipse([hx - hr * 1.85, hy - hr * 1.85, hx + hr * 1.85, hy + hr * 1.85],
              fill=(15, 17, 21, 255))
    d.ellipse([hx - hr, hy - hr, hx + hr, hy + hr], fill=dot)
    return img
...
def main():
    save(render(BLUE, GREEN), OUT)
    save(render(OPEN_RING, OPEN_DOT), OUT_OPEN)
